...
{
    // 填充广播响应头(7字节0xFE + 1字节0xAA)
    for(int i = 0; i < 7; i++)
        __DR_TX_BUF(dr_line)[i] = 0xFE;
    __DR_TX_BUF(dr_line)[7] = 0xAA;

    // 填充设备UID(6字节，每字节后添加交替的0xAA/0x55)
    for(int i = 0; i < 6; i++) {
        uint8_t byte = (__DR_RDM_DEVICE_INFO(dr_line).uid >> (8*(5-i))) & 0xFF;
        __DR_TX_BUF(dr_line)[8+i*2] = byte | 0xAA;
        __DR_TX_BUF(dr_line)[9+i*2] = byte | 0x55;
    }
    ...
    // 填充校验和(4字节，每字节后添加交替的0xAA/0x55)
    for(int i = 0; i < 2; i++) {
        uint8_t byte = (check_sum >> (8*(1-i))) & 0xFF;
        __DR_TX_BUF(dr_line)[20+i*2] = byte | 0xAA;
        __DR_TX_BUF(dr_line)[21+i*2] = byte | 0x55;
    }
...
void rdm_disc_mute_respone(dr_line_t *dr_line, uint64_t uid)
{
    ...
    rdm_package_set_cmd(0x11, 0x0002);  // DISC_MUTE命令
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x11, 0x0003);  // DISC_UN_MUTE命令
    uint8_t data[2] = {0x00, 0x00};
    rdm_package_set_data(2, data);
    rdm_package_send(dr_line);
}
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x21, 0x00C0);  // GET_DEVICE_VERSION命令
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x21, 0x0060);  // GET_DEVICE_INFO命令
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x21, 0x1000);  // GET_DEVICE_FLAG命令
    rdm_package_set_data(1, &__DR_RDM_DEVICE_INFO(dr_line).flag);
    rdm_package_send(dr_line);
}
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x31, 0x1000);  // SET_DEVICE_FLAG命令
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x21, 0x00F0);  // GET_DMX_ADDRESS命令
...
{
    rdm_package_set_uid(uid);
    rdm_package_set_cmd(0x31, 0x00F0);  // SET_DMX_ADDRESS命令
    rdm_package_set_data(0, NULL);
    rdm_package_send(dr_line);
}
...
void rdm_input_unpack(dr_line_t *dr_line)
{
    ...
    {
        // 检查标准RDM包头 (0xCC 0x01)
        if(__DR_RX_BUF(dr_line)[i] == 0xCC && __DR_RX_BUF(dr_line)[i+1] == 0x01)
            break;
    }
    ...
    {
        // 根据命令类别处理不同类型的包
        switch(__DR_RX_BUF(dr_line)[i+20])  // 命令类别字节
        {
            // 设备发现类命令 (0x10)
            case 0x10:
                // 设备唯一性搜索命令
                if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0x01)
                {
                    ...
                    // 解析搜索范围的低UID和高UID
                    uint64_t low_uid = ((uint64_t)__DR_RX_BUF(dr_line)[i+24] << 40) |
                                      ((uint64_t)__DR_RX_BUF(dr_line)[i+25] << 32) |
                                      ((uint64_t)__DR_RX_BUF(dr_line)[i+26] << 24) |
                                      ((uint64_t)__DR_RX_BUF(dr_line)[i+27] << 16) |
                                      ((uint64_t)__DR_RX_BUF(dr_line)[i+28] << 8)  |
                                       (uint64_t)__DR_RX_BUF(dr_line)[i+29];
                    
                    uint64_t high_uid = ((uint64_t)__DR_RX_BUF(dr_line)[i+30] << 40) |
                                       ((uint64_t)__DR_RX_BUF(dr_line)[i+31] << 32) |
                                       ((uint64_t)__DR_RX_BUF(dr_line)[i+32] << 24) |
                                       ((uint64_t)__DR_RX_BUF(dr_line)[i+33] << 16) |
                                       ((uint64_t)__DR_RX_BUF(dr_line)[i+34] << 8)  |
                                        (uint64_t)__DR_RX_BUF(dr_line)[i+35];
                ...
                // 设备哑音命令
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0x02)
                {
                    __DR_RDM_PARSE(dr_line).rdm_package = DISC_MUTE;
                }
                // 设备解除哑音命令
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0x03)
                {
            ...
            case 0x20:
                // 获取设备状态
                if(__DR_RX_BUF(dr_line)[i+21] == 0x10 && __DR_RX_BUF(dr_line)[i+22] == 0x00)
                {
                    __DR_RDM_PARSE(dr_line).rdm_package = GET_DRIVER_FLAG;
                }
                // 获取DMX地址
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0xF0)
                {
                    __DR_RDM_PARSE(dr_line).rdm_package = GET_DRIVER_DMX_ADDR;
                }
                // 获取设备版本
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0xC0)
                {
                    __DR_RDM_PARSE(dr_line).rdm_package = GET_DRIVER_VERSION;
                }
                // 获取设备信息
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0x60)
                {
                    __DR_RDM_PARSE(dr_line).rdm_package = GET_DRIVER_INFO;
                }
                // 获取设备模式
                else if(__DR_RX_BUF(dr_line)[i+21] == 0x00 && __DR_RX_BUF(dr_line)[i+22] == 0xE0)
                {
        ...
        // 记录数据部分
        uint8_t data_len = __DR_RX_BUF(dr_line)[i+23];
        memcpy(__DR_RDM_PARSE(dr_line).data, &__DR_RX_BUF(dr_line)[i+24], data_len);

        // 解析源UID (6字节)
        __DR_RDM_PARSE(dr_line).source_uid = 
            ((uint64_t)__DR_RX_BUF(dr_line)[i+9] << 40) |
            ((uint64_t)__DR_RX_BUF(dr_line)[i+10] << 32) |
            ((uint64_t)__DR_RX_BUF(dr_line)[i+11] << 24) |
            ((uint64_t)__DR_RX_BUF(dr_line)[i+12] << 16) |
            ((uint64_t)__DR_RX_BUF(dr_line)[i+13] << 8)  |
             (uint64_t)__DR_RX_BUF(dr_line)[i+14];
        
        // 计算并验证校验和
        uint32_t sum = 0;
        for(int j = i; j < i+24+data_len; j++)
        {
            sum += __DR_RX_BUF(dr_line)[j];
        }
        
        if(((sum >> 8) & 0xFF) != __DR_RX_BUF(dr_line)[i+24+data_len] || 
           (sum & 0xFF) != __DR_RX_BUF(dr_line)[i+25+data_len])
        {
            __DR_RDM_PARSE(dr_line).rdm_package = ERROR_PACKAGE;
        }
    }
    // 目标UID不匹配，不需要响应
    else
