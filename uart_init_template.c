...
#define DR_LINE1_DMA_CLK_ENABLE()      __HAL_RCC_DMA1_CLK_ENABLE()
...
#define DR_LINE1_TX_AF                 GPIO_AF2_USART1
#define DR_LINE1_RX_PORT               GPIOB
#define DR_LINE1_RX_PIN                GPIO_PIN_7
#define DR_LINE1_RX_AF                 GPIO_AF2_USART1
...
#define DR_LINE1_RX_DMA_IRQN           DMA1_Channel2_IRQn
#define DR_LINE1_TX_DMA_IRQHANDLER     DMA1_Channel1_IRQHandler
#define DR_LINE1_RX_DMA_IRQHANDLER     DMA1_Channel2_IRQHandler
...
{
    if(dr_line == dr_line1) {
        HAL_UART_Abort(&dr_line1_uart);  // 中止当前UART传输
    }
}
...
{
    if(dr_line == dr_line1) {
        HAL_UART_Receive_DMA(&dr_line1_uart, data, size);  // DMA方式接收
    }
}
...
void dr_line1_init(void)
{
    ...
    if(dr_line1 != NULL)
        dr_line_mode_set(dr_line1, DMX_OUTPUT);
}
