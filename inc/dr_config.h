...
#if USE_HAL_DRIVER
/**
 * @brief PY32F4xx HAL库支持
 * @note 使用AL库时必须定义USE_HAL_DRIVER宏
 */
...
/* RDM设备标识配置 */
#define RDM_DEFAULT_UID       0x19BA00000000UL  ///< 默认厂商UID（E1.20标准格式）

/* 定时器参数（单位：ms） */
#define IDLE_TIM_OVER         4     ///< 链路空闲检测超时阈值（ms）
#define SEND_DELAY_TIM_OVER   0     ///< 数据包间延迟（ms）
#define REFRESH_TIM_OVER      500   ///< 链路信号断开检测时间（ms）
#define RDM_RECV_TIM_OVER     100   ///< RDM接收超时时间（ms）
...
/* 输出设备配置 ---------------------------------------------------------*/
#define RDM_SEND_QUEUE_LENGTH 10    ///< RDM发送队列深度（单位：数据包）
