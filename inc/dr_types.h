...
typedef struct rdm_send_queue_t {
    ...
    uint8_t send_cnt;                   // 发送次数,超过三次判定为发送失败
...
typedef struct rdm_line_t {
    ...
    uid_t *quick_add;                      // 快速添加UID节点指针（输出设备专用）
 ...
 */
enum recv_statu {
    DR_LINE_RECV_ERROR = 0x01,     // 接收错误数据标志位
    DR_LINE_RECV_BREAK = 0x02,     // 接收到BREAK信号标志位
};
...
#define __DR_RECV_STATUS(dr_line)          dr_line->system_flag.recv_status
...
#define __DR_RDM_QUICK_ADD(dr_line)        dr_line->rdm_line.quick_add
